...
export function connectPlayerToServer(playerManager: PlayerManager, roomManager: RoomManager, player: Player, room: Room): void {
    ...
    player.sendMessage(IndividualMessageEventCode.PlayerInformation, { PlayerId: player.id, PlayerUsername: player.username, IsHost: player.isHost })

    // Send a separate message to the player containing information about the room (i.e. who is in it)
    const playerData: {Id:  string, Username: string, Position: {x: number, y: number, dir: number}, IsHost: boolean}[] = roomManager.getAllPlayersInRoom(room)
    .filter((playerInRoom: Player) => playerInRoom.id !== player.id )
    .map((otherPlayer: Player) => {
        return { Id: otherPlayer.id, Username: otherPlayer.username, Position: {x: otherPlayer.x, y: otherPlayer.y, dir: otherPlayer.dir }, IsHost: otherPlayer.isHost}
    });
